...
RUN INSTALL_PKGS="tar unzip bc which lsof java-1.8.0-openjdk java-1.8.0-openjdk-devel rh-nodejs6" && \
    yum install -y --enablerepo=centosplus --enablerepo=rhel-server-rhscl-7-rpms $INSTALL_PKGS && \
    rpm -V $INSTALL_PKGS && \
    yum clean all -y && \
